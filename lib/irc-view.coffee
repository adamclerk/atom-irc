...
class IrcView extends ScrollView
  ...
  addMessage: (from, to, message) =>
    ...
    appendFunction = => ircOutput.append line.text(util.format '<span class="ts">%s</span> <span class="un #{from}">%s</span> <span class="message">%s</span>', new Date().toLocaleTimeString(), from, message)
